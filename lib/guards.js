...
/**
 * This can be used to create a string guard - function that may be used to
 * perform run time type checks on the values. Function takes an optional
 * `defaultValue` that will be returned by a created guard if it's invoked
 * without (or with `undefined`) `value` argument. The `defaultValue` will
 * fallback to `undefined` if not provided. Function also takes another
 * optional argument `message` that represents a template of a message of
 * the `TypeError` that will be thrown by guard if it's invoked with incorrect
 * value.
 ...
 * var user1 = gUser("Jack");       // -> "Jack"
 * var user2 = gUser();             // -> "Anonymous"
 * var user3 = gUser(7);            // -> throws String expected instead of number `7`.
 * var gHello = guard.String("Hi", "string expected instead of {{type}}");
 * var msg = gHello(function() {}); // -> throws string expected instead of function.
 */
